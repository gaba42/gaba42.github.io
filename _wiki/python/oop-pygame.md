...
### 직접 접근 문제
인스턴스 변수에 직접 접근했을 시 문제인 상황
1. 인스턴스 변수의 이름을 변경해야 하는 상황
2. 인스턴스 변수를 계산된 값으로 만들어야 하는 상황
3. 데이터 검증이 필요한 상황

decorator를 사용하면 인스턴스 변수의 값을 설정하고 가져올 때 추가로 일련의 검증 작업이 이뤄지게 할 수 있다/ 인스턴스 변수에 대한 직접 접근을 허용하지 않아 캡슐화로도 볼 수 있다.  
하지만 코드를 읽을 때 이게 인스턴스 변수에 직접 접근하고 있는건지 프로퍼티로 접근하는지 불분명해 모호하다.
...
subclass는 2가지 방식으로 base class를 확장할 수 있다.
1. base class에 정의된 메서드 재정의
2. base class에 없는 신규 메서드 및 인스턴스 변수
...
- **콜백**: 특정 행동, 이벤트, 조건 발생에 따라 호출되는 객체의 함수 또는 메서드
- **클라이언트**: 클래스로 객체를 만들고 해당 객체의 메서드를 호출하는 모든 종류의 프로그램
- **게터(getter)**: 클래스로 만든 객체 데이터를 조회하는 메서드
- **세터(setter)**: 클래스로 만든 객체의 데이터 값을 할당하는 메서드
- **프로퍼티(속성)**: 클라이언트 입장에서 보면 인스턴스 같겠지만 사실은 접근할 때 특정 메서드를 대신 호출하는 클래스 속성이다.
- **추상화(abstraction)**: 불필요한 세부 사항을 숨겨서 복잡도를 낮추는 기법
- **기반 클래스(base class)**: 상속할 클래스. 하위 클래스 기능의 기반이 된다.
- **하위 클래스(subclass)**: 상속하는 클래스. 기반 클래스를 강화한다
